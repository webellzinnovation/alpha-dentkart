        ...
        <div className="flex items-center gap-8 md:gap-16 px-4 md:px-8">
          {CATEGORIES.map((cat) => (
            <button 
              ...
              className="flex-shrink-0 group flex items-center gap-3 opacity-60 hover:opacity-100 transition-all duration-300 cursor-pointer focus:outline-none"
            >
              <i className={`${cat.iconClass} text-2xl text-gray-500 dark:text-gray-400 group-hover:text-primary transition-colors`}></i>
              <span className="font-bold text-lg text-gray-500 dark:text-gray-400 group-hover:text-primary transition-colors tracking-tight whitespace-nowrap">{cat.name}</span>
            </button>
          ))}
        </div>
