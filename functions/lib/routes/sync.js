...
async function syncProducts() {
    ...
    for (const product of products) {
        ...
        const productRef = firebase_1.db.collection('products').doc(String(product.id));
...
async function syncOrders() {
    ...
    for (const order of orders) {
        ...
        const orderRef = firebase_1.db.collection('orders').doc(String(order.id));
...
async function syncUsers() {
    ...
    for (const customer of customers) {
        ...
        const userRef = firebase_1.db.collection('users').doc(String(customer.id));
...
async function syncCategories() {
    ...
    for (const cat of categories) {
        ...
        const catRef = firebase_1.db.collection('categories').doc(String(cat.id));
...
async function syncBrands() {
    ...
    for (const tag of tags) {
        ...
        const brandRef = firebase_1.db.collection('brands').doc(String(tag.id));
