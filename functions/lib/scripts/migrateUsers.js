...
async function migrateUsers() {
    ...
    const dataPath = path_1.default.join(process.cwd(), 'migration', 'wordpress_users.json');
    if (!fs_1.default.existsSync(dataPath)) {
        console.error(`❌ Data file not found at: ${dataPath}`);
        console.log('Please export your WordPress users to JSON.');
        process.exit(1);
    }
    const users = JSON.parse(fs_1.default.readFileSync(dataPath, 'utf8'));
    ...
    for (const wpUser of users) {
        try {
            ...
            let uid;
            try {
                const userRecord = await firebase_1.auth.getUserByEmail(wpUser.email);
                uid = userRecord.uid;
            ...
            await firebase_1.db.collection('users').doc(uid).set(userDoc, { merge: true });
