...
const createPromotionalTile = async (req, res) => {
    try {
        ...
        const cleanTile = Object.fromEntries(Object.entries(newTile).filter(([_, v]) => v !== undefined));
        const docRef = await firebase_1.db.collection('promotional_tiles').add(cleanTile);
