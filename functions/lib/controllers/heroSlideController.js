...
const createHeroSlide = async (req, res) => {
    try {
        ...
        const cleanSlide = Object.fromEntries(Object.entries(newSlide).filter(([_, v]) => v !== undefined));
        const docRef = await firebase_1.db.collection('hero_slides').add(cleanSlide);
