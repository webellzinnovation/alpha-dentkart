...
export const checkPincodeServiceability = async (req: Request, res: Response) => {
  try {
    ...
    if (!shiprocketService.validateIndianPincode(pincode)) {
      return res.status(400).json({
        success: false,
    ...
    const serviceability = await shiprocketService.checkPincodeServiceability(pincode);
...
export const getEstimatedDelivery = async (req: Request, res: Response) => {
  try {
    ...
    const estimation = await shiprocketService.getEstimatedDelivery(
      pickupPincode,
      deliveryPincode,
      weight
    );
...
export const createShiprocketOrder = async (req: Request, res: Response) => {
  try {
    ...
    const shiprocketOrderData = shiprocketService.createOrderRequest(orderData);

    // Create order in Shiprocket
    const response = await shiprocketService.createOrder(shiprocketOrderData);
...
export const trackShipment = async (req: Request, res: Response) => {
  try {
    ...
    if (awbNumber) {
      trackingData = await shiprocketService.trackShipment(awbNumber);
    } else if (orderId) {
      trackingData = await shiprocketService.trackOrder(orderId);
    } else {
      return res.status(400).json({
        success: false,
...
export const cancelShiprocketOrder = async (req: Request, res: Response) => {
  try {
    ...
    const response = await shiprocketService.cancelOrder(orderIds);
...
export const getAvailableCouriers = async (req: Request, res: Response) => {
  try {
    const { deliveryPincode, weight = 0.5, pickupPincode = '110001' } = req.body;
...
export const calculateShippingCharges = async (req: Request, res: Response) => {
  try {
