...
const firebase_1 = require("../config/firebase"); // Firestore, Auth, Admin SDK
...
async function updateUserByEmail(req, res) {
    try {
        ...
        let userSnapshot = await firebase_1.db.collection('users')
        ...
        if (userSnapshot.empty) {
            ...
            const allUsers = await firebase_1.db.collection('users').get();
            for (const doc of allUsers.docs) {
                const userEmail = (doc.data().email || '').toLowerCase().trim();
                if (userEmail === searchEmail) {
                    userSnapshot = { docs: [doc], empty: false };
                    break;
                }
            }
        }
        ...
        if (userSnapshot.empty) {
            ...
            try {
                const userRecord = await firebase_1.auth.getUserByEmail(searchEmail);
                ...
                if (userDoc.exists) {
                    ...
                    const newUserRef = firebase_1.db.collection('users').doc(userRecord.uid);
                    ...
                    userSnapshot = { docs: [newDoc], empty: false };
                }
            }
            catch (authError) {
        ...
        if (updates.disabled !== undefined) {
            try {
                const userRecord = await firebase_1.auth.getUserByEmail(searchEmail);
                await firebase_1.admin.auth().updateUser(userRecord.uid, { disabled: updates.disabled });
        ...
        const updatedUser = { id: updatedDoc.id, ...updatedDoc.data() };
        logger_1.default.info('User updated by admin via email', { email: searchEmail, userId });
        res.json({ message: 'User updated successfully', user: updatedUser });
    }
    catch (error) {
        console.error('❌ Error updating user:', error);
        logger_1.default.error('Error updating user by email', { error });
...
async function getAllUsers(req, res) {
    try {
        ...
        if (pageToken) {
            ...
            const result = await firebase_1.auth.listUsers(limit, pageToken);
            ...
            // Get first batch
            const firstResult = await firebase_1.auth.listUsers(limit);
            ...
            if (getTotal) {
                ...
                let countPageToken = firstResult.pageToken;
                let count = firstResult.users.length;
                while (countPageToken) {
                    const countResult = await firebase_1.auth.listUsers(1000, countPageToken);
        ...
        if (authUsers.length > 0) {
            ...
            for (let i = 0; i < authUsers.length; i += batchSize) {
                ...
                try {
                    const snapshots = await firebase_1.db.collection('users')
                        .where(firebase_1.admin.firestore.FieldPath.documentId(), 'in', uids)
                        .get();
