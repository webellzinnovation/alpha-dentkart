 ...
 *
 * Usage:
 *   npx tsx backend/src/scripts/migration/incremental-sync.ts
 */
...
async function fetchAll(endpoint, params = {}) {
    ...
    do {
        try {
            ...
            console.log(`  ⚠️ Error on page ${page}: ${err.message}`);
            break;
        }
...
async function syncCustomers() {
    ...
    for (const c of wpCustomers) {
        ...
        // Determine user type from billing data
        const billing = c.billing || {};
        const hasCompany = !!billing.company;
        // Infer user type - if company exists, likely a business/supplier
        let userType = hasCompany ? 'supplier' : 'regular';
        // If company contains dental/doctor keywords, might be a dental clinic
        if (hasCompany) {
            const companyLower = billing.company.toLowerCase();
            if (companyLower.includes('dental') || companyLower.includes('clinic') || companyLower.includes('hospital') || companyLower.includes('doctor')) {
                userType = 'dental-doctor';
            }
        }
        const hasAddress = !!billing.address_1;
        // Mark as verified if they have billing address or are existing customer
        const isVerified = hasAddress || (c.date_created && new Date(c.date_created) < new Date());
        let uid = null;
        try {
...
async function syncOrders() {
    ...
    console.log(`  � found ${wpOrders.length} new orders`);
