...
export class ShiprocketService {
    ...
    async getShippingRates(pickupPostcode: string, deliveryPostcode: string, weight: number, cod: boolean) {
        // Mock rates
        return [
            {
    ...
    async getEstimatedDelivery(pickupPostcode: string, deliveryPostcode: string, weight: number) {
        return {
    ...
    async createOrder(shiprocketOrderData: any) {
        return {
            status_code: 1,
    ...
    async cancelOrder(shiprocketOrderIds: string[]) {
        return {
            status_code: 1,
    ...
    async getAvailableCouriers(pickupPostcode: string, deliveryPostcode: string, weight: number) {
        return [
            {
                courier_name: 'Blue Dart',
                rate: 100,
    ...
    async printManifest(shipmentId: string) {
        return { manifest_url: 'http://example.com/manifest.pdf' };
    }
